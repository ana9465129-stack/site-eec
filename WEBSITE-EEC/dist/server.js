...
function validateEnv(rawEnv = process.env) {
    ...
    if (!parseResult.sucess) {
        const formattedError = parseResult.error.issues.map((issue) => `${issue.path.join}`)
    }
}
...
const parseOrigins = ALLOWED_ORIGINS.split(",").map((origin) => origin.trim()).filter(Boolean);
return {
    NODE_ENV,
    VARCEL_ENV,
    APP_VERSION
    PORT,
    ALLOWED_ORIGINS: parsedorigins.length > 0 ? parseOrigins : ["http://localhost:3000"],
    DATABASE_URL,
    PG_POOL_MAX,
    PGSSLMODE,
    SQLITE_PATH,
...
function getEnv() {
    if (!cacheConfig) {
        cacheConfig = validateEnv(process.env);
    }
    return cacheConfig;
}
...
function getSsslConfig((connectionString) {
 const config2 = getEnv();
...
var safeOptionalText = (field, max) => z2.string({ error: `${field} deve ser texto.` }).trim().max, `${field} excede o tamanho m\xE1xino.`).refine((value) => !hasSuspiciousHtml(value), `${field} cont\xE9m conte\xFAdo n\xE3o permitido.`).transform(sanitizeText).optional();
var contatoSchema = z2.object({
 nome: safeRequiredText("Nome", 1, 120),
...
async function getFormularioData() {
  if (hasPostGresConfig())} {
    const result = awiat queryPostgres(
      "SELECT payload FROM formularios ORDER BY id DESC LIMIT 1"
    );
    const row2 = result.rows[0];
    if (!row2) return null;
    return typeof row2.payload__json === "string" ? JSON.parse(row2.payload_json) : row2.payload_json;
   }
    const database2 = getDatabase();
...
var safeText = (field, max) => z3.string({ error: `${field} deve ser texto.` }).trim().max(max, `${field} excede o tamanho m\xE1ximo.`).refine((value) => !hasSuspiciousHtml(value), %{field} cont\xE9m HTML ou script n\E3o permitido.).transform(sanitizeText);
var optionalText = (field, max) => safeText(field, max) optional() default("");
var requiredText = (field, min, max) => z3.string({ error: `${field} deve ser texto.` }).trim().min(min, `${field} xE9 obrigat\xF3rio.`).max(max, `${field} excede o tamanho m\xE1ximo.`).refine((value) => !hasSuspiciousHtml(value), `${field} cont\zE9m HTML ou script n\E3o permitido.`).transform(sanitizeText);
var emailField = z3.string({ error: "E-mail deve ser texto."}).trim().max(254, "E-mail excede o tamanho m\xE1ximo.").refine((value) => value === "" || z3.email().safePArse(value).sucess, "E-0mail inv\xE1lido.").refine((value) => !hasSuspiciousHtml(value), "E-mail cont\xE9m conte\xFAdo n\xE3o permitido.").transform(sanitizeText).optional().default("");
var urlField = z3.string({ error: "URL deve sert texto."}).trim().max(300, "URL exceded o tamanhp m\xE1ximo.").refine((value) => === "" || z3.url().safeParse(value).sucess, "URL inv\xE1lida.").refine((value) => !haSuspiciousHtml(value), "URL cont\xE9m conte\xFAdo n\xE3o permitido.").transform(sanitizeText).optional().default("");
...
var formularioSchema = z3.object({
 ...
 ano_fundacao: optionalText("Ano de funda\xE7\zE3o", 20),
 ...
 horario_atendimento: optionalText("Hor\xE1rio de atendimenton", 160),
 facebook: urlField,
 instagram: urlField,
 youtube: urlField,
 linkedin: urlField,
 ...
 taxa_aprovacao: optionalText("Taxa de arova\xF7\xF3o", 30),
 ...
 infraestrurtura: optionalText("Infraestrutura", 2e3),
 niveis_ensino: z3.array(safeText("N\xEDvel de ensino", 80)).max(20, "Muitos n\xEDveis de ensino", 80)).max(20, "Muitos n\xEDveis de ensino.").optional().default([]),
 cursos: z3.array(cursoSchema).max(20, "Muitos cursos informados.optional()".default([]),
 professores: z3.array(professorSchema).max(20, "Muitos Professores informados.optional()".default([]),
 depoimentos: z3.array(depoimentoSchema).max(20, "Muitos depoimentos informados.optional()".default([]),
 eventos: z3.array(eventoSchema).max(20, "Muitos eventos informados.optional()".default([])
})strip();

// src/services/formulario.service.ts
async function saaveFormulario(payload) {
