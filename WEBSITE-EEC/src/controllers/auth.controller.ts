...
export async function postLogin(c: Context) { // cria e exporta a função responsável pelo login
    try {
        ...
        // A sessão é estabelecida por cookies HttpOnly seguros gerenciados pelo servidoor.
        // Nenhum token de acesso é exposto no corppo do payload JSON.
        return c.json({
            sucess: true,
            user
        })
        ...
        return c.json({ error: 'Erro ao processar autenticaçãao.' }, 500) 
    }
